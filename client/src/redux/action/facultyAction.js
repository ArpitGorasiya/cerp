...
const facultyAddAssignmentFlag = (data) => {
    return {
        type: "FACULTY_ADD_ASSIGNMENT_FLAG",
        payload: data
    }
}

export const facultyAddAssignment = (assignmentCredential) => {
    return async (dispatch) => {
        try {
            const { data } = await axios({
                method: 'Post',
                url: url + "/api/faculty/addAssignment",
                data: assignmentCredential
            })
            dispatch(facultyAddAssignmentFlag(true))
