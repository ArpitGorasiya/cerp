...
const addAssignment = require('../validation/addAssignment')
...
module.exports = {
    ...
    getAllNoticef: async (req, res, next) => {
        try {
            const allNotice = await Notice.find({ $or: [{ type: "faculty" }, { type: "student & faculty" }] }).exec()
    ...
    addAssignment: async (req, res, next) => {
        try {
            // const { errors, isValid } = addAssignment(req.body)
            // if (!isValid) {
            //     return res.status(400).json(errors)
            // }
            const { name, subject, sDate, image, year, section, reg } = req.body


            // VALIDATE REQUEST BODY
            // if (!name || !subject || !sDate || !image || !year || !section) {
            //     return res.status(400).json({ success: false, message: "Probably you have missed certain fields" })
            // }

            const result = cloudinary.uploader.upload(image, { folder: "assignments" })
            console.log(result)
            ...
            return res.status(200).json({ success: true, message: "Assignment Add successfully", response: newAssignment })
        }
        catch (error) {
            return res.status(400).json({ success: false, message: error.message })
        }
    },
    getAllAssignmentf: async (req, res, next) => {
        try {
            // const { reg } = req.body
            // if (!reg) {
            //     return res.status(400).json({ success: false, message: "no found" })
            // }
